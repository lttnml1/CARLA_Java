...
class World(object):
    ...
    def is_in_crosswalk(self, crosswalk, actor_location):
...
def game_loop(args):
    ...
    try:
        ...
        grid = Grid(client.get_world(),-65,-100,-10,30)#, draw_time = 10)   
        ...
        for j in range (0,len(outer_polygon)-1):
            world.world.debug.draw_line(outer_polygon[j]+carla.Location(z=1), outer_polygon[j+1]+carla.Location(z=1), thickness=0.2, color=carla.Color(255,0,0), life_time=20)
        ...
        for j in range (0,len(inner_polygon)-1):
            world.world.debug.draw_line(inner_polygon[j]+carla.Location(z=1), inner_polygon[j+1]+carla.Location(z=1), thickness=0.2, color=carla.Color(0,255,0), life_time=20)
        
        #for loc in outer_polygon:
        #    world.world.debug.draw_point(loc,size=0.2,color=carla.Color(255,0,0),life_time=10)
        
        
        #read the path the Adversary is supposed to take from the file
        if "CategoricalDistrib" in args.file:
        ...
        while True:
            ...
            #if(world.is_in_crosswalk(cw,Adversary3_loca)):
            #    print("**ADVERSARY3 IN CROSSWALK")
